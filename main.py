...
def login():
    ...
    if form.validate_on_submit():
        ...
        if not user:
            ...
            flash("Password didn't match please try again")
            return redirect(url_for('login'))
        else:
            login_user(user)
            return redirect(url_for("get_all_posts"))
...
def show_post(post_id):
    ...
    if form.validate_on_submit():
        if not current_user.is_authenticated:
            flash("You need to login first to post this comment")
            return redirect(url_for('login'))
        else:
            new_comment = Comment(
            ...
            db.session.commit()
    return render_template("post.html", form=form, current_user=current_user, post=requested_post)
